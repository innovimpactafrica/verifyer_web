...
interface MissionItem {
    ...
    date: string;
    description: string;
    status: 'En attente' | 'Assignée' | 'Terminée' | 'Annulée';
}
...
export class MissionDetailComponent {
    ...
    // Small mock dataset (no service by request)
    private missions: MissionItem[] = [
        {
            id: 'MS-006',
            ...
            client: 'Aminata Sow',
            date: '2025-09-23',
            description: 'Vérification logement R+4',
            status: 'En attente'
        },
        {
            id: 'MS-002',
            ...
            client: 'Mame Diouf',
            date: '2025-07-14',
            description: 'Contrôle hôtelier annuel',
            status: 'Assignée'
        },
        {
            id: 'MS-003',
            ...
            client: 'Fatou Diop',
            date: '2025-08-10',
            description: 'Audit hygiène cuisine',
            status: 'Assignée'
        }
    ];
    ...
    // UI helpers
    statusPillClasses(status: MissionItem['status']): string {
        switch (status) {
            case 'Assignée':
                return 'text-emerald-700 bg-[#0D823B0D]';
            case 'En attente':
    ...
    selectAgent(a: AgentItem) {
        this.selectedAgent = a;
    }

    // Match missions list icon/background logic
    ...
    getBienSvg(type: string): SafeHtml {
        ...
        switch (type) {
            case 'Immeuble':
                svg = `<svg width="30" height="30" viewBox="0 0 22 21" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                break;
            case 'Hôtel':
                svg = `<svg width="30" height="30" viewBox="0 0 22 15" fill="none" xmlns="http://www.w3.org/2000/svg">
