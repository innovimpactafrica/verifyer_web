...
export class DetailPaiementComponent implements OnInit {
    ...
    private demandes = [
        {
            ...
            agent: 'Fatou Sall',
            dateVisite: '2025-11-05',
            montant: '120 000 F CFA',
            paiement: {
                numeroFacture: 'FACT-145',
                datePaiement: '10/09/2025',
        ...
        },
        {
            ...
            agent: 'Aminata Ba',
            dateVisite: '2025-10-15',
            montant: '180 000 F CFA',
            paiement: {
                numeroFacture: 'FACT-145',
                datePaiement: '10/09/2025',
                modePaiement: 'Carte bancaire',
    ...
    ngOnInit(): void {
        // Récupérer l'ID depuis l'URL et les données de paiement depuis l'état de navigation
        this.route.paramMap.subscribe(params => {
            this.demandeId = params.get('id') || '';
            this.loadDemandeDetails();
        });

        // Récupérer les détails de paiement depuis l'état de navigation (si fournis)
        const navigation = window.history.state;
        if (navigation && navigation.paymentDetails) {
            this.paymentDetails = navigation.paymentDetails;
        }
    }

    loadDemandeDetails(): void {
        // Charger les détails de la demande selon l'ID
        this.demandeDetails = this.demandes.find(d => d.id === this.demandeId);

        if (!this.demandeDetails) {
            console.warn(`Demande ${this.demandeId} introuvable`);
        } else if (!this.paymentDetails && this.demandeDetails.paiement) {
            // Si les détails de paiement ne sont pas fournis dans la navigation, utiliser ceux de la demande
            this.paymentDetails = this.demandeDetails.paiement;
        }
    }
    ...
    switch (statut) {
      ...
      case 'Soumis':
        return 'bg-[#274B9B14] text-[#274B9B]';
      case 'En cours':
  ...
  getIconBgClass(type: string): string {
    ...
    downloadInvoice(): void {
        ...
        console.log('Téléchargement de la facture:', this.paymentDetails?.numeroFacture);
